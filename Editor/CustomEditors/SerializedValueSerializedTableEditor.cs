...
namespace Zor.EventBasedBlackboard.Serialization
{
	...
	public sealed class SerializedValueSerializedTableEditor : Editor
	{
		...
		private void OnEnable()
		{
			...
			{
				BlackboardDebug.LogError($"{targetType.FullName} is derived from {typeof(SerializedValueSerializedTable_Base).FullName}. Instead it should be derived from {typeof(SerializedValueSerializedTable<>).FullName}");
				return;
			}
		...
		private static bool IsSubclassOfRightClass(Type toCheck)
		{
			while (toCheck != null && toCheck != typeof(object))
			{
				...
				if (typeof(SerializedValueSerializedTable<>) == type)
				{
					return true;
				}
